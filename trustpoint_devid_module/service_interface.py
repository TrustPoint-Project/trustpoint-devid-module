...
from trustpoint_devid_module.serializer import CertificateSerializer
...
from trustpoint_devid_module.schema import KeyInventory, KeyInventoryEntry, EnumeratedPublicKeys
from trustpoint_devid_module.schema import CertificateInventory, CertificateInventoryEntry
...
class DevIdModule:
    ...
    def __init__(self, working_dir: str | Path) -> None:
        ...
        self._certificate_inventory_path = self.working_dir / 'certificate_inventory.json'
        self._certificate_chain_inventory_path = self.working_dir / 'certificate_chain_inventory.json'
    ...
    def initialize(self) -> None:
        ...
        with open(self.certificate_inventory_path, 'w') as f:
            f.write(CertificateInventory(next_available_index=0, certificates=[]).model_dump_json())
    ...
    def _check_data_consistency_shallow(self) -> None:
        ...
        if not self.key_inventory_path.exists() or not self.key_inventory_path.is_file():
            raise DevIdModuleCorrupted
        if not self.certificate_inventory_path.exists() or not self.certificate_inventory_path.is_file():
            raise DevIdModuleCorrupted

    def check_data_consistency_deep(self) -> None:
        pass
    ...
    def generate_ldevid_key(self, key_type: str) -> int:
        ...
        if key_type == KeyType.SECP521R1:
            return self._generate_secp_r1_ldevid_key(key_size=521)
    ...
    @staticmethod
    def _get_key_type_from_private_key(private_key: PrivateKeySerializer) -> KeyType:
        private_key = private_key.as_crypto()
        print(private_key)
        if isinstance(private_key, rsa.RSAPrivateKey):
            if private_key.key_size == 2048:
                return KeyType.RSA2048
            elif private_key.key_size == 3072:
                return KeyType.RSA3072
            elif private_key.key_size == 4096:
                return KeyType.RSA4096
            else:
                raise ValueError('RSA with unsupported key length found.')
        ...
        if isinstance(private_key, ec.EllipticCurvePrivateKey):
            if isinstance(private_key.curve, ec.SECP256R1):
                ...
                return KeyType.SECP521R1
            else:
                raise ValueError('EC with unsupported curve found.')
    ...
    def insert_ldevid_key(self, private_key: PrivateKeySerializer) -> int:
        # get bytes in DER format, sha256 fingerprints and file names of both the private and public key
        private_key_bytes = private_key.as_pkcs8_der()
        ...
        public_key_bytes = private_key.public_key_serializer.as_der()
        ...
        # get key type
        key_type = self._get_key_type_from_private_key(private_key)

        # check if private key already exists
        if self._get_key_entry_by_public_key_sha256_fingerprint(public_key_sha256_fingerprint) is not None:
        ...
        new_key_inventory_entry = KeyInventoryEntry(
            ...
            enabled=True,
            subject_public_key_info=key_type.value,
        ...
        with open(self.working_dir / private_key_file_name, 'wb') as f:
            f.write(private_key_bytes)
        with open(self.working_dir / public_key_file_name, 'wb') as f:
            f.write(public_key_bytes)

        return new_key_inventory_entry.key_index

    def insert_ldevid_certificate(self, certificate: CertificateSerializer) -> int:
    ...
    def pprint_key_inventory(self) -> None:
        key_inventory = self._get_key_inventory()
        print(key_inventory.model_dump_json(indent=4))

    def pprint_certificate_inventory(self) -> None:
        certificate_inventory = self._get_certificate_inventory()
        print(certificate_inventory.model_dump_json(indent=4))
    ...
    def _store_certificate_inventory(self, certificate_inventory: CertificateInventory) -> None:
    ...
    def _get_key_entry_by_key_index(self, key_index: int) -> None | KeyInventoryEntry:
        keys_with_key_index = [
            key_entry for key_entry in self._get_key_inventory().keys
    ...
    def _get_key_entry_by_public_key_sha256_fingerprint(
            self,
            public_key_sha256_fingerprint: str) -> None | KeyInventoryEntry:
    ...
    def _get_certificate_entry_by_certificate_index(self, certificate_index: int) -> None | CertificateInventoryEntry:
        certificate_with_certificate_index = [
            certificate_entry for certificate_entry in self._get_certificate_inventory().certificates
    ...
    def _get_certificate_entry_by_certificate_sha256_fingerprint(
            self, certificate_sha256_fingerprint: str) -> None | CertificateInventoryEntry:

        certificates_with_sha256_fingerprint = [
            certificate_entry for certificate_entry in self._get_certificate_inventory().certificates
            if certificate_entry.certificate_sha256_fingerprint == certificate_sha256_fingerprint
        ]
    ...
    def _get_key_entry_by_certificate_index(self, certificate_index: int) -> None | KeyInventoryEntry:
        certificate_entry = self._get_certificate_entry_by_certificate_index(certificate_index=certificate_index)
        return self._get_key_entry_by_key_index(certificate_entry.key_index)

    def _get_certificate_entries_by_key_index(self, key_index: int) -> list[CertificateInventoryEntry]:
        return [
            certificate_entry for certificate_entry in self._get_certificate_inventory().certificates
